...
| `REPO_NAME`             | Repository name as one URL-encoded path segment                | `my-demo-repository` or `pierre%2Fexample`       |
...
export REPO_NAME="my-demo-repository"   # URL-encode names containing "/": pierre/example -> pierre%2Fexample
...
| Get note for commit           | GET      | `/repos/{repo_name}/notes?sha=SHA`            | `git:read`      |
...
Legacy `/api/v1/...` paths remain served for backward compatibility but are
deprecated; new integrations must use the paths above.
...
```bash
# Create credential
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/git-credentials" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"username":"git","password":"ACCESS_TOKEN_OR_PASSWORD"}'

# Update credential
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/git-credentials/CREDENTIAL_ID" -X PUT \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"username":"git","password":"ROTATED_ACCESS_TOKEN"}'

# Delete credential
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/git-credentials/CREDENTIAL_ID" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/branches/create" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/branches?limit=20&cursor=CURSOR" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/branches/diff?branch=BRANCH&base=main&path=src/foo.go" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/merge" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/merge/preview?source_branch=feature/demo&target_branch=main&include_content=true" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/branches" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commits?branch=main&path=docs/guide.md&limit=20&cursor=CURSOR" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commit?sha=COMMIT_SHA" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/diff?sha=COMMIT_SHA&baseSha=OPTIONAL_BASE&gitApplyCompatible=true&path=src/foo.go" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/restore-commit" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/files?ref=main&path=docs&recursive=false&limit=200" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/files/metadata?ref=main&path=src&limit=100" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/file?path=src/main.go&ref=main" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"

# Fetch just metadata (no body)
curl -I "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/file?path=src/main.go&ref=main" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/blame?path=src/main.go&ref=main&range=10,30&range=/getUser/,+30&detect_moves=true" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/grep" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "ref": "main",
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/archive" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/tags" -X POST \
...
# List tags
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/tags?limit=20&cursor=CURSOR" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"

# Delete tag
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/tags/v1.0.0" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
# Create note
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/notes" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" -H "Content-Type: application/json" \
...
# Get note
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/notes?sha=COMMIT_SHA" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"

# Delete note
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/notes" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" -H "Content-Type: application/json" \
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/notes?sha=COMMIT_SHA&ref=reviews" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/notes/refs?prefix=reviews/&limit=20" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/pull-upstream" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/base" -X DELETE \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
```
...
```bash
# First page
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commits?branch=main&limit=20"

# Subsequent pages — use next_cursor from previous response
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commits?branch=main&limit=20&cursor=NEXT_CURSOR_VALUE"
```
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commit-pack" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/files?ref=preview/pr-42&ephemeral=true" \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN"
...
curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/diff-commit" -X POST \
  -H "Authorization: Bearer $CODE_STORAGE_TOKEN" \
  -H "Content-Type: application/x-ndjson" --data-binary @-
```
...
```bash
# Fetch current HEAD
CURRENT_SHA=$(curl "$CODE_STORAGE_BASE_URL/repos/$REPO_NAME/commits?limit=1" \
...
| Notes                 | Attach metadata to commits without modifying commit SHA. Default ref `refs/notes/commits`; pass `ref` to target another `refs/notes/*` ref (custom refs must be enabled server-side). List refs via `GET /repos/{repo_name}/notes/refs`. |
