  ...
  describe('Repo getBranchDiff', () => {
    it.each([{ merge_base_sha: 'common-ancestor' }, {}, { merge_base_sha: '' }])(
      ...
      async (metadata) => {
        ...
        mockFetch.mockImplementationOnce((url) => {
          const requestUrl = new URL(url as string);
          expect(requestUrl.pathname).toBe('/api/repos/repo-branch-diff-ephemeral-base/branches/diff');
  ...
  describe('Repo getCommitDiff', () => {
    it.each([
      ...
      async (metadata) => {
        ...
        mockFetch.mockImplementationOnce((url) => {
          const requestUrl = new URL(url as string);
          expect(requestUrl.pathname).toBe('/api/repos/repo-commit-diff/diff');
          expect(requestUrl.searchParams.getAll('path')).toEqual([
            'new.txt',
            'image.png',
          ]);
